...
Given(/^I have placed all of my ships$/) do
  in_browser(:one) do
    ...
    select('Aircraft Carrier (length: 5)', :from => 'ship')
    select('5', :from => 'row')
    select('A', :from => 'column')
    select('vertical', :from => 'orientation')
    click_button('Place Ship')
    select('Destroyer (length: 3)', :from => 'ship')
    select('5', :from => 'row')
    select('B', :from => 'column')
    select('vertical', :from => 'orientation')
    click_button('Place Ship')
    select('Submarine (length: 3)', :from => 'ship')
    select('5', :from => 'row')
    select('C', :from => 'column')
    select('vertical', :from => 'orientation')
    click_button('Place Ship')
    select('Battleship (length: 4)', :from => 'ship')
    select('5', :from => 'row')
    select('D', :from => 'column')
    select('vertical', :from => 'orientation')
    click_button('Place Ship')
    select('Patrol Boat (length: 2)', :from => 'ship')
    select('5', :from => 'row')
